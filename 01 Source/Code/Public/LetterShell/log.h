...
#define     ERROR_TEXT          CSI(31) "Error(%lldms)%s:" CSI(39)  /**< 错误标签 */
#define     WARNING_TEXT        CSI(33) "Warn (%lldms)%s:" CSI(39)  /**< 警告标签 */
#define     INFO_TEXT           CSI(32) "Info (%lldms)%s:" CSI(39)  /**< 信息标签 */
#define     DEBUG_TEXT          CSI(34) "Debug(%lldms)%s:" CSI(39)  /**< 调试标签 */
#define     VERBOSE_TEXT        CSI(36) "Verbo(%lldms)%s:" CSI(39)  /**< 冗余信息标签 */
#define     DEFAULT_TEXT        CSI(39)                             /**< 解决shell输出被修改颜色 */
#else
...
#define logFormat(text, level, fmt, ...) \
        if (LOG_ENABLE) {\
            logWrite(LOG_ALL_OBJ, level, text " " fmt "" DEFAULT_TEXT LOG_END, \
